...
- เรียก `scan_directory` ผ่าน Scan MCP
...
- Task และ permission เก็บใน memory และหายเมื่อ restart
- Agent ทำงานหนึ่ง tool call ต่อรอบเป็นหลัก
- ยังไม่มี authentication หรือ multi-user isolation (Swagger ไม่มี auth scheme เพราะยังไม่มีระบบ auth จริง)
- Stop ป้องกันขั้นถัดไป แต่ยังยกเลิก HTTP request ที่ส่งไป Ollama ระหว่างทางไม่ได้
- Ollama response ยังเป็นแบบ non-streaming; SSE ใช้ส่ง lifecycle events และ final message
