...
export class AgentService {
  ...
  private async run(taskId: string): Promise<void> {
    ...
    if (latestUserMessage && evaluateToolPolicy(latestUserMessage).isMutation) {
      this.complete(taskId, FILE_MUTATION_UNAVAILABLE_RESPONSE, 0, true);
      return;
    }
    const memoryRecords = this.memory.getContextFor(task.workspacePath);
    const memoryContextPrompt = this.memory.buildContextPrompt(memoryRecords);
    ...
    const messages: OllamaChatMessage[] = [
      ...
      ...(memoryContextPrompt
        ? [{ role: 'system' as const, content: memoryContextPrompt }]
        : []),
      {
        role: 'system',
  ...
  private complete(
    ...
    if (!skipMemoryExtraction) {
      void this.extractMemoriesFor(taskId, content).catch(() => {
        // Extraction is a best-effort enrichment - the turn already
        // completed successfully regardless of whether this succeeds.
      });
    }
  }

  private async extractMemoriesFor(
    taskId: string,
    ...
    const existingContext = this.memory.buildContextPrompt(
      this.memory.getContextFor(task.workspacePath),
    );
    const candidates = await this.ollama.extractMemories(
      latestUserMessage,
      assistantContent,
      existingContext,
    );
    if (candidates?.length) {
      this.memory.applyExtracted(candidates, task.workspacePath, taskId);
    }
  }
